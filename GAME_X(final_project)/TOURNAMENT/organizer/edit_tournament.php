...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ownTournament) {
    ...
    $reg_end_date = $_POST['reg_deadline'];
    $status = $_POST['status'];

    $stmt = $conn->prepare("
        UPDATE tournaments SET
            title=?, game=?, description=?, start_date=?, end_date=?,
            max_teams=?, reg_start_date=?, reg_end_date=?, status=?
...
<head>
    ...
    <title>Edit Tournament - Game X</title>
...
<body>
    ...
    <form method="POST">
        ...
        <input type="date" name="start_date" value="<?= $tournament['start_date'] ? date('Y-m-d', strtotime($tournament['start_date'])) : '' ?>" required>

        <label>End Date</label>
        <input type="date" name="end_date" value="<?= $tournament['end_date'] ? date('Y-m-d', strtotime($tournament['end_date'])) : '' ?>">

        <label>Max Teams</label>
        <input type="number" name="max_teams" min="2" value="<?= $tournament['max_teams'] ?>">

        <label>Registration Start</label>
        <input type="date" name="reg_start_date" value="<?= $tournament['reg_start_date'] ? date('Y-m-d', strtotime($tournament['reg_start_date'])) : '' ?>">

        <label>Registration Deadline</label>
        <input type="date" name="reg_deadline" value="<?= $tournament['reg_end_date'] ? date('Y-m-d', strtotime($tournament['reg_end_date'])) : '' ?>">
